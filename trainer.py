...
class Trainer(object):
    def __init__(self,train_set,eval_set,test_set,args,device):
        ...
        self.model=self.model.from_pretrained(MODEL_PATH[args.model_type],self.config,args=args,
                                              intent_num_labels=len(self.intent_vocab),
    ...
    def train_model(self):
        ...
        if self.args.max_step>0:
            ...
            self.args.num_train_epochs=t_toal//(len(data_iter)//self.grad_accumulate_step)
        else:
            t_total=self.args.num_train_epochs*(len(data_iter)//self.grad_accumulate_step)
        ...
        lr_scheduler=get_linear_schedule_with_warmup(optimizer,self.args.warm_steps,t_toal)
        ...
        for _ in train_pgb:
            ...
            for step,batch in enumerate(epochs_pgb):
                ...
                #load batch_data
                batch=(t.to(self.device) for t in batch)#put tensor to gpu or cpu
