...
MODEL_Classes={
    'jointBert':(BertConfig,BertTokenizer,modeling_JointBert.JointBert),
}
MODEL_PATH={
    'jointBert':'bert-base-uncased',
}
def get_intent_labels(args):
    return [labels.strip() for labels in open(os.path.join(args.data_dir,args.task,args.intent_label_files),'r',encoding='utf_8')]
def get_slot_labels(args):
    return [labels.strip() for labels in open(os.path.join(args.data_dir,args.task,args.slot_label_files),'r',encoding='utf_8')]
...
def get_model_metrics(predict_intent,predict_slots,intent_labels,slot_labels):
    assert len(predict_intent)==len(predict_slots)==len(intent_labels)==len(slot_labels)
    result={}
    ...
    semantic_result=get_sentence_frame_acc(predict_intent,predict_slots,intent_labels,slot_labels)
